...
class OppdragPrintNextTableAction extends BaseDataTableAction
{
    ...
    public function applyFilter(ActiveQuery $query, $columns, $search)
    {
        $query
            ...
            ->with(['user', 'propertyPrint', 'propertyVisits' => function (ActiveQuery $query) {
                $query->orderBy([
                    'property_visits.til' => SORT_DESC
                ]);
            }]);
        ...
        foreach ($columns as $column) {
            ...
            if ($column['name'] === 'do_not_show_nybygg' && $searchValue === 'true') {
                $query->andWhere(['not', [
                    'property_details.tinde_oppdragstype' => 'Prosjekt'
                ]]);
            }

            if ($column['name'] === 'do_not_show_grimsoen' && $searchValue === 'true') {
                $query->joinWith(['department' => function (ActiveQuery $query) {
                    $query->filterWhere(['not', ['department.partner_id' => 5]]);
                }]);
            }
        }
    ...
    public function applyOrder(ActiveQuery $query, $columns, $order)
    {
        ...
        foreach ($order as $key => $item) {
            ...
            if (in_array($columnName, $printTableColumns)) {
                $query->joinWith('propertyPrint')
                    ->addOrderBy([
                        "property_print.{$columnName}" => $sort
                    ]);
