...
$lead = array(
    'name'         => $model->name,
    'url'          => $url . '/clients/detaljer/'.$model->id,
    'type'         => $model->type,
    'address'      => $model->propertyDetails ? $model->propertyDetails->adresse : '',
    'regStamp'     => $registrertTimeStamp,
    'regDate'      => strftime("%d. %B %Y %H:%M ", $registrertTimeStamp),
    'regDays'      => round((time() - $registrertTimeStamp) / 60 / 60 / 24),
    'delegateStamp'=> $delegertTimeStamp,
    'delegateDate' => strftime("%d. %B %Y %H:%M ", $delegertTimeStamp),
    'delegateDays' => round((time() - $delegertTimeStamp) / 60 / 60 / 24),
    'user' => array(
        'name' => "Joachim Schala",
        'firm' => "Carl Berner",
