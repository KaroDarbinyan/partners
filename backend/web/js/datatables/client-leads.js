window.addEventListener('load', function () {
  $('#data-table').DataTable({
    ...
    columns: [
      ...
      {title: 'Eiendom', data: 'property'}, // 3
      {title: 'Registrert', data: 'created_at'}, // 4
      {title: 'Sist endret', data: 'latestLog.created_at'}, // 5
      {data: 'latestLog.type'}, // 6
    ...
    columnDefs: [
      {targets: [0, 2, 6, 8, 9, 10, 11], visible: false},
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          ...
          badge = `<span class="schala-status d-block">${badge}</span>`;

          return `${badge} <b class="d-block">${data.oppdragsnummer}</b> 
            <a href="${window.DATA_TABLE.url_to_property}/${data.id}">${data.adresse}</a>`;
        }
      },
      {
        targets: 4,
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          ...
          return `<span class="schala-status">
                        <span class="m-badge m-badge--dot schala-status-${row.latestLog.type}"></span>
                        <em>${row.latestLog.type}</em>
                    </span><span class="table-schala-time">${date.format('lll')}</span><span class="table-schala-time-ago">${date.fromNow()}</span>`;
        }
      },
      {
