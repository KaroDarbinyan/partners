let GoogleMap = {
    radiusWasChangedTimeout: null,
    circleWasChangedTimeout: null,
    init: function () {
        let radiusInput = $('#filter_map_circle_radius');
        let mapCoordinates = $('#filter_map_coordinates');
        let mapParams = mapCoordinates.val().split(';');

        let center = new google.maps.LatLng(
            parseFloat(mapParams[0] || 59.9139),
            parseFloat(mapParams[1] || 10.7522),
        );
       ...
       const newStyle = new google.maps.StyledMapType(
        ...
        let mapCircle = new google.maps.Circle({
            center: center,
            radius: parseInt(radiusInput.val()) || 1000,
            map: map,
            strokeColor: 'orange',
            strokeOpacity: .9,
            fillColor: 'orange',
        ...
        radiusInput.change(function () {
            if (this.radiusWasChangedTimeout) {
                clearTimeout(this.radiusWasChangedTimeout)
            }

            const val = parseInt($(this).val())

            this.radiusWasChangedTimeout = setTimeout(function () {
                mapCircle.setRadius(val)
        ...
        $('#google-map-search').change(function () {
            setTimeout(function () {
                let mapParams = mapCoordinates.val().split(';')
                console.log(mapParams)
                let lat = parseFloat(mapParams[0] || 59.9139)
                let lng = parseFloat(mapParams[1] || 10.7522)
    ...
    circleWasChanged: function (circle) {
        if (this.circleWasChangedTimeout) {
            clearTimeout(this.circleWasChangedTimeout)
        }

        this.circleWasChangedTimeout = setTimeout(function () {
            const {center, radius} = circle

            $('#filter_map_coordinates')
                .val(center.lat() + ';' + center.lng() + ';' + radius)
                .change()
        }, 200)
    },
